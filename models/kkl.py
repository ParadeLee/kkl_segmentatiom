...
mlp = ConvMLP(blocks=[2, 4], dims=[128, 256], mlp_ratios=[2, 2], channels=64, n_conv_blocks=2)
...
class ARNN2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        drb1 = self.relu_refine1_3(self.bn_refine1_3(self.conv_refine1_3(d1_2)))

        # drb 2
        d2_2 = F.interpolate(h4, size=[h3.size(2), h3.size(3)], mode='bilinear', align_corners=True)
        drb2 = self.relu_refine2_3(self.bn_refine2_3(self.conv_refine2_3(d2_2)))

        # drb 3
        drb3 = self.relu_refine3_3(self.bn_refine3_3(self.conv_refine3_3(h3)))

        # drb 4
        d4_2 = F.interpolate(h2, size=[h3.size(2), h3.size(3)], mode='bilinear', align_corners=True)
        drb4 = self.relu_refine4_3(self.bn_refine4_3(self.conv_refine4_3(d4_2)))

        # drb 5
        d5_2 = F.interpolate(h1, size=[h3.size(2), h3.size(3)], mode='bilinear', align_corners=True)
        drb5 = self.relu_refine5_3(self.bn_refine5_3(self.conv_refine5_3(d5_2)))
